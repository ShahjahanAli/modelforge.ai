...
$line = & $py $script --audio $audio --model "bengaliAI/tugstugi_bengaliai-regional-asr_whisper-medium" --device cpu --compute-type int8 --beam-size 5 --best-of 5 --temperature 0 --no-speech-threshold 0.35 --no-vad-filter --language bn 2>$null | Select-Object -Last 1
$j = $line | ConvertFrom-Json
Write-Host "fresh PS len=$($j.text.Length) segs=$($j.segments.Count) open=$($j.text.Substring(0, [Math]::Min(40, $j.text.Length)))"
